...
Zmodem.CRC = (function() {
    ...
    //http://mirror.fsf.org/pmon2000/2.x/src/pmon/loaders/zmodem/crctab.c
    const crc32tab = [ /* CRC polynomial 0xedb88320 */
    ...
    //http://mirror.fsf.org/pmon2000/2.x/src/pmon/loaders/zmodem/crctab.h
    function _updcrc32(b, c) {
        var left = crc32tab[(c ^ b) & 0xff];
        var right = (c >> 8) & 0x00ffffff;

        //JavaScript screws up bit-shifting sometimes, e.g.:
        //  2405801727 >> 1 = -944582785
        //
        //...so we accommodate:

        var lefthigh = Math.floor(left / 65536);
        var leftlow = left & 0xffff;

        var righthigh = Math.floor(right / 65536);
        var rightlow = right & 0xffff;

        var rethigh = lefthigh ^ righthigh;
        var retlow = leftlow ^ rightlow;

        return( rethigh * 65536 + retlow );

        //return (crc32tab[(c ^ b) & 0xff] ^ ((c >> 8) & 0x00FFFFFF));
    }
     ...
     * @returns {Array} crc - The CRC, expressed as an array of octet values.
     */
    function crc32(octet_nums) {
        var crc = 0xffffffff;
        for (var b=0; b<octet_nums.length; b++) {
            crc = _updcrc32( octet_nums[b], crc );
        }

        crc = 0xffffffff - crc;     //i.e., crc = ~crc

        return Zmodem.ENCODELIB.pack_u32_le(crc);
    }
     ...
     */
    function verify16(bytes_arr, got) {
